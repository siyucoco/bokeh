...
def deriv1(t, y, params):
   ...
   # T_n, co2_n, q_n, T_n2, co2_n2, q_n2, T_n3, co2_n3, q_n3, T_n4, co2_n4, q_n4, T_n5, co2_n5, q_n5 == y
    ...
    T1dot = -ener_balan(v0, theta, deltZ) * T_n + ener_balan(v0, theta, deltZ) * T_in + ener_balan2(episl_r) * (
        R_co2(T_n, co2_n, q_n))/theta + ener_balan3(a_s, T_n)/theta
   
    co2_1dot = -mass_balan(v0, episl_r, deltZ) * co2_n + mass_balan(v0, episl_r, deltZ) * c_co2_0 - (
        R_co2(T_n, co2_n, q_n)) * masss_balan2(episl_r, ps)/episl_r
    q1dot = R_co2(T_n, co2_n, q_n)
    # print(f"energy balance in T1", {ener_balan(v0, theta, deltZ)})
    
    T2dot = -ener_balan(v0, theta, deltZ) * T_n2 + ener_balan(v0, theta, deltZ) * T_n + ener_balan2(episl_r) * (
        R_co2(T_n2, co2_n2, q_n2))/theta + ener_balan3(a_s,  T_n2)/theta
    # print(f"T2", {T2})
    co2_2dot = -mass_balan(v0, episl_r, deltZ) * co2_n2 + mass_balan(v0, episl_r, deltZ) * co2_n - (
        R_co2(T_n2, co2_n2, q_n2)) * masss_balan2(episl_r, ps)/episl_r
    q2dot = R_co2(T_n2, co2_n2, q_n2)
    # print(f"energy balance in T1", {ener_balan(v0, theta, deltZ)})

    T3dot = -ener_balan(v0, theta, deltZ) * T_n3 + ener_balan(v0, theta, deltZ) * T_n2 + ener_balan2(episl_r) * (
        R_co2(T_n3, co2_n3, q_n3))/theta + ener_balan3(a_s, T_n3)/theta
    co2_3dot = -mass_balan(v0, episl_r, deltZ) * co2_n3 + mass_balan(v0, episl_r, deltZ) * co2_n2 - (
        R_co2(T_n3, co2_n3, q_n3)) * masss_balan2(episl_r, ps)/episl_r
    q3dot = R_co2(T_n3, co2_n3, q_n3)

    T4dot = -ener_balan(v0, theta, deltZ) * T_n4 + ener_balan(v0, theta, deltZ) * T_n3 + ener_balan2(episl_r) * (
        R_co2(T_n4, co2_n4, q_n4))/theta + ener_balan3(a_s,  T_n4)/theta
    co2_4dot = -mass_balan(v0, episl_r, deltZ) * co2_n4 + mass_balan(v0, episl_r, deltZ) * co2_n3 - (
        R_co2(T_n4, co2_n4, q_n4)) * masss_balan2(episl_r, ps)/episl_r
    q4dot = R_co2(T_n4, co2_n4, q_n4)

    T5dot = -ener_balan(v0, theta, deltZ) * T_n5 + ener_balan(v0, theta, deltZ) * T_n4 + ener_balan2(episl_r) * (
        R_co2(T_n5, co2_n5, q_n5))/theta + ener_balan3(a_s,  T_n5)/theta
    co2_5dot = -mass_balan(v0, episl_r, deltZ) * co2_n5 + mass_balan(v0, episl_r, deltZ) * co2_n4 - (
        R_co2(T_n5, co2_n5, q_n5)) * masss_balan2(episl_r, ps)/episl_r
...
# ----------------------- Extract Numbers from ODE results ---------------------- #
...
co2_convert = [co2_1dot, co2_2dot, co2_3dot, co2_4dot, co2_5dot]
print(co2_convert)
