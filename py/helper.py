...
def deriv(t, y):
   ...
   # T_n, co2_n, q_n, T_n2, co2_n2, q_n2, T_n3, co2_n3, q_n3, T_n4, co2_n4, q_n4, T_n5, co2_n5, q_n5 == y
    ...
    T1 = -ener_balan(theta, deltZ) * T_n + ener_balan(theta, deltZ) * T0 + ener_balan2(episl_r) * (
        R_co2(T_n, co2_n, q_n)) + ener_balan3(a_s, Tw, T_n)
    ...
    T2 = -ener_balan(theta, deltZ) * T_n + ener_balan(theta, deltZ) * T1 + ener_balan2(episl_r) * (
        R_co2(T_n, co2_n, q_n)) + ener_balan3(a_s, Tw, T_n)
    co2_2 = -mass_balan(episl_r, deltZ) * co2_n + mass_balan(episl_r, deltZ) * co2_1 - (
        R_co2(T_n, co2_n, q_n)) * masss_balan2(episl_r, ρs)
    q_2 = R_co2(T_n, co2_n, q_n)

    T3 = -ener_balan(theta, deltZ) * T_n + ener_balan(theta, deltZ) * T2 + ener_balan2(episl_r) * (
        R_co2(T_n, co2_n, q_n)) + ener_balan3(a_s, Tw, T_n)
    co2_3 = -mass_balan(episl_r, deltZ) * co2_n + mass_balan(episl_r, deltZ) * co2_2 - (
        R_co2(T_n, co2_n, q_n)) * masss_balan2(episl_r, ρs)
    q_3 = R_co2(T_n, co2_n, q_n)

    T4 = -ener_balan(theta, deltZ) * T_n + ener_balan(theta, deltZ) * T3 + ener_balan2(episl_r) * (
        R_co2(T_n, co2_n, q_n)) + ener_balan3(a_s, Tw, T_n)
    co2_4 = -mass_balan(episl_r, deltZ) * co2_n + mass_balan(episl_r, deltZ) * co2_3 - (
        R_co2(T_n, co2_n, q_n)) * masss_balan2(episl_r, ρs)
    q_4 = R_co2(T_n, co2_n, q_n)

    T5 = -ener_balan(theta, deltZ) * T_n + ener_balan(theta, deltZ) * T4 + ener_balan2(episl_r) * (
        R_co2(T_n, co2_n, q_n)) + ener_balan3(a_s, Tw, T_n)
