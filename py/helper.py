...
def ener_balan2(episl_r):
    ener_balan2 = (1 - episl_r) * ρs * deltH_co2
    return (ener_balan2)
...
# change it to q
def R_co2(T, c_co2, q, b_var, t_var, qs_var):
    # b_var = b(T)
    # t_var = t_h(T)
    # # print(t_var)
    # qs_var = q_s(T)
...
def deriv1(t, y, params):
   ...
   # T_n, co2_n, q_n, T_n2, co2_n2, q_n2, T_n3, co2_n3, q_n3, T_n4, co2_n4, q_n4, T_n5, co2_n5, q_n5 == y
    ...
    co2_1 = -mass_balan(v0, episl_r, deltZ) * co2_n + mass_balan(v0, episl_r, deltZ) * c_co2_0 - (
        R_co2(T_n, co2_n, q_n,  b_var, t_var, qs_var)) * masss_balan2(episl_r, ρs)
    ...
    T2 = -ener_balan(v0, theta, deltZ) * T_n2 + ener_balan(v0, theta, deltZ) * T1 + ener_balan2(episl_r) * (
        R_co2(T_n2, co2_n2, q_n2,  b_var, t_var, qs_var)) + ener_balan3(a_s,  T_n2)
    co2_2 = -mass_balan(v0, episl_r, deltZ) * co2_n2 + mass_balan(v0, episl_r, deltZ) * co2_1 - (
        R_co2(T_n2, co2_n2, q_n2,  b_var, t_var, qs_var)) * masss_balan2(episl_r, ρs)
    q_2 = R_co2(T_n2, co2_n2, q_n2,  b_var, t_var, qs_var)

    T3 = -ener_balan(v0, theta, deltZ) * T_n3 + ener_balan(v0, theta, deltZ) * T2 + ener_balan2(episl_r) * (
        R_co2(T_n3, co2_n3, q_n3,  b_var, t_var, qs_var)) + ener_balan3(a_s, T_n3)
    co2_3 = -mass_balan(v0, episl_r, deltZ) * co2_n3 + mass_balan(v0, episl_r, deltZ) * co2_2 - (
        R_co2(T_n3, co2_n3, q_n3,  b_var, t_var, qs_var)) * masss_balan2(episl_r, ρs)
    q_3 = R_co2(T_n3, co2_n3, q_n3,  b_var, t_var, qs_var)

    T4 = -ener_balan(v0, theta, deltZ) * T_n4 + ener_balan(v0, theta, deltZ) * T3 + ener_balan2(episl_r) * (
        R_co2(T_n4, co2_n4, q_n4,  b_var, t_var, qs_var)) + ener_balan3(a_s,  T_n4)
    co2_4 = -mass_balan(v0, episl_r, deltZ) * co2_n4 + mass_balan(v0, episl_r, deltZ) * co2_3 - (
        R_co2(T_n4, co2_n4, q_n4,  b_var, t_var, qs_var)) * masss_balan2(episl_r, ρs)
    q_4 = R_co2(T_n4, co2_n4, q_n4,  b_var, t_var, qs_var)

    T5 = -ener_balan(v0, theta, deltZ) * T_n5 + ener_balan(v0, theta, deltZ) * T4 + ener_balan2(episl_r) * (
        R_co2(T_n5, co2_n5, q_n5,  b_var, t_var, qs_var)) + ener_balan3(a_s,  T_n5)
    co2_5 = -mass_balan(v0, episl_r, deltZ) * co2_n5 + mass_balan(v0, episl_r, deltZ) * co2_4 - (
        R_co2(T_n5, co2_n5, q_n5,  b_var, t_var, qs_var)) * masss_balan2(episl_r, ρs)
    q_5 = R_co2(T_n5, co2_n5, q_n5,  b_var, t_var, qs_var)
